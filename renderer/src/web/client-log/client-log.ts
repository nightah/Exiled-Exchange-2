...
export function parseLine(line: string) {
  ...
  const channelPatterns = new Map<RegExp, MessageChannel>([
    [_$.CHAT_GLOBAL, MessageChannel.GLOBAL],
    [_$.CHAT_TRADE, MessageChannel.TRADE],
    [_$.CHAT_SYSTEM, MessageChannel.SYSTEM],
    [_$.CHAT_WHISPER_TO, MessageChannel.WHISPER_TO],
    [_$.CHAT_WHISPER_FROM, MessageChannel.WHISPER_FROM],
    [_$.CHAT_PARTY, MessageChannel.PARTY],
    [_$.CHAT_GUILD, MessageChannel.GUILD],
  ]);
  ...
  for (const [pattern, channel] of channelPatterns) {
    ...
    if (match) {
      entry = {
        channel,
        guildTag: match.groups?.guild_tag,
  ...
  if (entry.channel === MessageChannel.WHISPER_FROM || entry.channel === MessageChannel.WHISPER_TO) {
    Host.sendEvent({
      name: "CLIENT->MAIN::game-log-variables",
...
function parseGeneratingLevel(line: string) {
  const match = line.match(/^Generating level (\d+) area "([^"]+)" with seed (\d+)$/);
  if (!match) return null;

  const [, level, areaName, seed] = match;
  return {
    level: Number(level),
    areaName: areaNameParser(areaName),
    seed: Number(seed),
  };
}
...
function parseTradeWhisper(body: string): TradeDetails | undefined {
  ...
  match = body.match(TRADE_BULK_WHISPER[lang]);
  if (match) {
    const [pAmount, pName] = splitFirstWord(match.groups?.price ?? "");
...
function areaNameParser(zoneID: string) {
  return WORLD_AREAS.get(zoneID) ?? zoneID;
}

function splitFirstWord(str: string): [string, string] {
